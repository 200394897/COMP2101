...
myhostname=$(hostname)
#here looking for  interface name
interface_name=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
#hre looking for a lan address
lan_address=$(ip a s $interface_name|awk '/inet /{gsub(/\/.*/,"");print $2}')
#here we are finding lan hostname by getent hosts and variable $lan_address
lan_hostname=$(getent hosts $(ip a s $interface_name)|awk '/inet /{gsub(/\/.*/,"");print $2}' | awk '{print $2}')
external_ip=$(curl -s icanhazip.com)
#here we are getting  an external IP address from the  curl
external_name=$(getent hosts $external_ip| awk '{print $2}')
#here getting an external ip from hosts file and $external_ip
router_address=$(ip r s default| cut -d ' ' -f 3)
#her we are looking  a default router address
router_hostname=$(getent hosts $router_address|awk '{print $2}')
#lookng for router and hostname with the ue of getent host and $routeraddress.
#here we are moving command to some different variables.
cat <<EOF
Hostname        : $myhostname
